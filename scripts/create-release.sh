...
update_roadmap() {
    ...
    if [ -f "$roadmap_file" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            git commit -m "chore: update ROADMAP.md to $VERSION_TAG
...
Co-Authored-By: Delta Operator <noreply@deltacli.dev>" > /dev/null 2>&1
            ...
            if [ $? -eq 0 ]; then
                log_success "Committed ROADMAP.md update"
                git push origin main > /dev/null 2>&1
                if [ $? -eq 0 ]; then
                    log_success "Pushed ROADMAP.md update to origin"
                else
                    log_warning "Failed to push ROADMAP.md update. Please push manually."
                fi
            else
                log_warning "Failed to commit ROADMAP.md update"
            fi
