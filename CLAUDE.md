...
**DO NOT manually create releases with git commands and GitHub CLI**. Always use the provided release script at `./scripts/create-release.sh` which handles all release automation including version validation, building, packaging, documentation updates, and GitHub release creation.

### Creating a New Release

1. **Prepare Release Documentation**:
   - Ensure all changes are documented in CHANGELOG.md under `## [Unreleased]` section
   - Create release notes file: `RELEASE_NOTES/RELEASE_NOTES_v<version>.md`
   - The CHANGELOG.md MUST have an `[Unreleased]` section (the script will fail without it)
...
2. **Commit All Changes**:
   ...
   Co-Authored-By: Delta Operator <noreply@deltacli.dev>"
   ```

3. **Create Git Tag** (REQUIRED before running script):
   ```bash
...
4. **Run the Release Script**:
   ```bash
   # Option 1: Create release locally (builds, packages, generates artifacts)
   ./scripts/create-release.sh v<version>
   
   # Option 2: Create release and upload to GitHub (requires gh CLI)
...
1. **Validates**: Checks version format, git tag existence, and clean repository state
2. **Updates Documentation**: 
   - Moves CHANGELOG.md `[Unreleased]` section to the new version
   - Updates ROADMAP.md with new version and date
   - Commits and pushes these changes automatically
3. **Builds**: Creates binaries for all supported platforms with version injection
4. **Packages**: Creates platform-specific archives (.tar.gz and .zip)
5. **Generates**: 
   - SHA256 checksums for all files
   - Release info documentation
   - i18n translation archive
6. **Uploads** (if --upload flag used): Creates GitHub release with all artifacts

### Release Script Requirements

- **Prerequisites**: 
  - Git tag MUST exist before running the script (e.g., `v0.4.6-alpha`)
...
- **Version Format**: Must follow semantic versioning: `vX.Y.Z` or `vX.Y.Z-suffix`
  - Examples: `v0.4.6-alpha`, `v1.0.0`, `v2.1.3-beta`

- **Generated Artifacts**:
  - Cross-platform binaries (Linux, macOS Intel/ARM, Windows)
  - Compressed archives (.tar.gz and .zip for each platform)
  - i18n translations archive
...
- **"Git tag does not exist"**: Create the tag first with `git tag -a v<version> -m "message"`
- **"Repository has uncommitted changes"**: Commit or stash changes before release
- **"CHANGELOG.md does not have [Unreleased] section"**: Add `## [Unreleased]` section to CHANGELOG.md
- **"Failed to build platform binary"**: Ensure `make build-all` works before running script
