...
build: $(OUTPUT_BINARY)
	...
	CGO_ENABLED=0 GOOS=$(TARGET_OS) GOARCH=$(TARGET_ARCH) go build $(GOFLAGS) -o $(OUTPUT_BINARY) ai.go ai_manager.go jump_manager.go jump_helper.go cli.go memory_manager.go memory_commands.go tokenizer.go tokenizer_commands.go help.go
